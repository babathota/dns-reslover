import message_formate as mf 
import upd_connect_sock as up 
...
''' 
see the  RFC 1035 Section
FOR SERCHING RECOD IN QTYPE USE THIS 
A = 0001
AAA = 001c
NS = 2 
CNAME = 5 
...
for ip in ips:
    ...
    if(response_id!=Header.id):
        ...
        ns, lst = res.decode_dns_responce(response)
        if ns==0:
            break
        ips.extend(lst)